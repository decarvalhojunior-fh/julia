...
- `a` enables ASCII mode (disables `UTF` and `UCP` modes). By default `\\B`, `\\b`, `\\D`,
  ...
  this option, these sequences only match ASCII characters. Additionally, this allows
  matching against invalid UTF-8 strings, by treating both matcher and target as simple
  bytes (e.g. as if they were Latin-1) instead of as character encodings. This option can be
  further refined by starting the pattern with (*UCP) or (*UTF). This option is often
  combined with `s`.
